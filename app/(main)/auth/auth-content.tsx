          ...
          <div className="w-full md:w-1/2">
            <Card className="w-full">
              <CardHeader>
                ...
                <CardDescription>Login to your account or create a new one to order and earn rewards</CardDescription>
              </CardHeader>
              <CardContent>
                    ...
                    {showForgotPassword ? (
                      <form onSubmit={handleForgotPassword} className="space-y-4">
                        ...
                        <div className="flex gap-2">
                          <Button type="button" variant="outline" onClick={() => setShowForgotPassword(false)} className="flex-1">Back to Login</Button>
                          <Button type="submit" className="flex-1 bg-[#d73a31] hover:bg-[#c73128]" disabled={isResetting}>
                      ...
                      <Form {...loginForm}>
                        <form onSubmit={loginForm.handleSubmit(onLoginSubmit)} className="space-y-4">
                          ...
                          <button type="button" onClick={() => setShowForgotPassword(true)} className="text-sm text-[#d73a31] hover:underline font-medium">
                            Forgot password?
                          </button>
                          <Button type="submit" className="w-full bg-[#d73a31] hover:bg-[#c73128]" disabled={loginMutation.isPending}>
                  ...
                  <TabsContent value="register">
                    <Form {...registerForm}>
                      <form onSubmit={registerForm.handleSubmit(onRegisterSubmit)} className="space-y-4">
                        ...
                        <FormField control={registerForm.control} name="marketingOptIn" render={({ field }) => (
                          <FormItem className="flex flex-row items-start space-x-3 space-y-0 py-4">
                            <FormControl>
                              <Checkbox checked={field.value} onCheckedChange={field.onChange} />
                            </FormControl>
                            <div className="space-y-1 leading-none">
                              <FormLabel>Subscribe to marketing emails for exclusive offers</FormLabel>
                            </div>
                          </FormItem>
                        )} />
                        <Button type="submit" className="w-full bg-[#d73a31] hover:bg-[#c73128]" disabled={registerMutation.isPending}>
          ...
          <div className="w-full md:w-1/2 bg-white rounded-lg overflow-hidden shadow-md">
            ...
            <div className="p-6">
              ...
              <ul className="space-y-4">
                {[
                  { num: '1', title: 'Track Your Orders', desc: 'View your order history and reorder your favorites with just a click.' },
                  { num: '2', title: 'Earn Rewards', desc: 'Earn points with every order and redeem them for exclusive rewards and discounts.' },
                  { num: '3', title: 'Faster Checkout', desc: 'Save your delivery information for a seamless ordering experience.' },
                ].map(({ num, title, desc }) => (
                  <li key={num} className="flex items-start">
                    <span className="flex-shrink-0 w-8 h-8 bg-[#d73a31] text-white rounded-full flex items-center justify-center mr-3">{num}</span>
                    <div>
            ...
            <div className="px-6 py-4 bg-gray-50 border-t border-gray-200">
              <p className="text-center text-gray-600">Already have a favorite order? Login to quickly reorder!</p>
            </div>
          </div>
        </div>
      </div>
