...
export async function GET(request: NextRequest) {
  try {
    ...
    const statusCounts = filteredOrders.reduce((counts: any, o: any) => {
      counts[o.status] = (counts[o.status] || 0) + 1;
      return counts;
    }, {});

    const orderTypeCounts = filteredOrders.reduce((counts: any, o: any) => {
    ...
    try {
      ...
      const itemSales = allOrderItems.reduce((items: any, oi: any) => {
        const itemName = oi.name || `Item ${oi.menuItemId}`;
        if (!items[itemName]) items[itemName] = { name: itemName, sales: 0, revenue: 0 };
        items[itemName].sales += oi.quantity;
        items[itemName].revenue += parseFloat(oi.price) * oi.quantity;
        return items;
      }, {});
    ...
    const uniqueCustomers = new Set(filteredOrders.map((o: any) => o.userId));
    const customerOrderCounts = filteredOrders.reduce((counts: any, o: any) => {
