...
class ShipDayService {
  ...
  private async makeRequest(endpoint: string, method: 'GET' | 'POST' | 'PUT' = 'GET', data?: any): Promise<any> {
    ...
    console.log(`[ShipDay] Making ShipDay API request: ${method} ${url}`, 'ShipDay');
    
    try {
      const response = await fetch(url, options);
      
      console.log(`[ShipDay] ShipDay API response status: ${response.status} ${response.statusText}`, 'ShipDay');
      
      const responseText = await response.text();
      console.log(`[ShipDay] ShipDay API response body: ${responseText}`, 'ShipDay');
      ...
      console.log(`[ShipDay] ShipDay API request failed: ${error}`, 'ShipDay');
      throw error;
    }
  }
  ...
  async createDeliveryOrder(orderData: ShipDayOrderData): Promise<ShipDayResponse> {
    try {
      ...
      console.log(`[ShipDay] Sending ShipDay payload: ${JSON.stringify(shipdayPayload, null, 2)}`, 'ShipDay');
      
      const response = await this.makeRequest('/orders', 'POST', shipdayPayload);
      
      console.log(`[ShipDay] ShipDay response: ${JSON.stringify(response, null, 2)}`, 'ShipDay');
      
      if (response.success) {
        console.log(`[ShipDay] ShipDay order created successfully: ${response.orderId}`, 'ShipDay');
        return {
          success: true,
      ...
      console.log(`[ShipDay] Failed to create ShipDay order: ${error.message}`, 'ShipDay');
      return {
        success: false,
        error: error.message
      };
  ...
  async getOrderStatus(shipdayOrderId: string): Promise<ShipDayOrderStatus> {
    try {
      ...
      console.log(`[ShipDay] ShipDay order status for ${shipdayOrderId}: ${orderStatus.orderStatus}`, 'ShipDay');
      return orderStatus;
    } catch (error: any) {
      console.log(`[ShipDay] Failed to get ShipDay order status: ${error.message}`, 'ShipDay');
      throw error;
    }
  }
  ...
  async getOrderTracking(orderNumber: string): Promise<{
    ...
    try {
      ...
      console.log(`[ShipDay] Failed to get order tracking for ${orderNumber}: ${error.message}`, 'ShipDay');
      throw error;
    }
  }
  ...
  async updateOrderStatus(shipdayOrderId: string, status: string): Promise<any> {
    try {
      ...
      console.log(`[ShipDay] Failed to update ShipDay order status: ${error.message}`, 'ShipDay');
      throw error;
    }
  }
  ...
  async cancelOrder(shipdayOrderId: string): Promise<any> {
    try {
      ...
      console.log(`[ShipDay] Failed to cancel ShipDay order: ${error.message}`, 'ShipDay');
      throw error;
    }
  }
  ...
  async handleWebhook(payload: any): Promise<void> {
    try {
      ...
      console.log(`[ShipDay] ShipDay webhook received for order ${orderId}: ${status}`, 'ShipDay');
      ...
      if (status === 'delivered') {
        console.log(`[ShipDay] Order ${orderId} has been delivered!`, 'ShipDay');
      } else if (status === 'out_for_delivery') {
        console.log(`[ShipDay] Order ${orderId} is out for delivery`, 'ShipDay');
      } else if (status === 'picked_up') {
        console.log(`[ShipDay] Order ${orderId} has been picked up by driver`, 'ShipDay');
      }
      
    } catch (error: any) {
      console.log(`[ShipDay] Error handling ShipDay webhook: ${error.message}`, 'ShipDay');
    }
  }
  ...
  async getDeliveryEstimate(address: ShipDayAddress): Promise<{
    ...
    try {
      ...
      console.log(`[ShipDay] Failed to get delivery estimate: ${error}`, 'ShipDay');
      return null;
    }
  }
}
